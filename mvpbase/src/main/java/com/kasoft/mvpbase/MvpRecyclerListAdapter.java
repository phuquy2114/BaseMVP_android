...
public abstract class MvpRecyclerListAdapter<M, P extends IPresenter, VH extends PresenterViewHolder<P>> extends MvpRecyclerAdapter<M, P, VH> {
    ...
    public void updateItem(M item) {
        ...
        if (position >= 0) {
            ...
            presenters.remove(modelId);

            addInternal(item, position);
        }
    ...
    private void addInternal(M item, int position) {
